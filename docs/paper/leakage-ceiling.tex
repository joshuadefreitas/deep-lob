...
In exploratory runs preceding this study we observed validation accuracy spanning
roughly $0.67$ to $0.75$ for a fixed configuration; those runs predate the
committed harness and are \emph{not} reproducible from this repository, so the
figure is reported as an observation rather than as evidence. A seeded
demonstration of the same effect is the obvious next artifact and is not yet
present.
