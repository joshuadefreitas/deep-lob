...
The table above is twenty split seeds on one random-walk realisation. To check
that the conclusion is not a property of that realisation, the full study was
repeated across five independent generator paths --- $900$ runs. Excess over
baseline, per path:
...
The across-path standard deviation of the excess at $h=20$ under
\texttt{random\_split} is $0.008$, against a within-path standard deviation
across model seeds of $0.078$. The effect is an order of magnitude more stable
across data realisations than across initialisations, which is consistent with
it being a property of the evaluation geometry rather than of any particular
fitted model.
